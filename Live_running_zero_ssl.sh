...
revoke_cert() {
    ...
    if [ "$success_response" = "1" ]; then
        ...
        echo "New certificate issued So no revocation needed."
         #echo "API Response: ${revoke_response}"
    fi
}
...
certificate_ids=$(echo "$list_response" | jq -r --arg common_name "$CERT_NAME" '.results[] | select(.common_name == $common_name and .status == "issued") | .id')

if [ -z "$certificate_ids" ]; then
    echo "No issued certificates found for common name '$CERT_NAME'."
    echo "Starting new certificate creation."
    gsr_generation
else
    for certificate_id in $certificate_ids; do
        ...
        if [ $days_difference -lt $RENEW_DAYS ]; then
            ...
            echo "The Script Executed Date Is : $current_date"
            echo "Certificate needs renewal. Renewal started."
            gsr_generation
        else
            current_date=$(date "+%Y-%m-%d %H:%M:%S")
            echo "The Script Executed Date Is : $current_date"
