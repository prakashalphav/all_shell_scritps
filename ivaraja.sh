...
text_httpd_logs() {
     ...
     httpd_access_dir="/home/ubuntu/log_file/"
    ...
    for access_log_zip in "$httpd_access_dir"access_log*; do
        if [ -f "$access_log_zip" ]; then
            ...
            log_file="${httpd_access_dir}zipped-Access-logs/${filename}.txt"
            echo -n > "$log_file"
...
zip_text_files() {
   ...
   zip_log_directory="/home/ubuntu/log_file/zipped-Access-logs/"
...
clear_old_httpd_logs() {
    access_dir="/home/ubuntu/log_file/"
delete_no_of_days_from="07"
delete_upto_no_of_days="100"
    for access_log in "$access_dir"access_log*; do
        ...
        if [ -f "$access_log" ]; then
            ...
            echo "No log files detected in $httpd_logs_directory"
        fi
    done
}
